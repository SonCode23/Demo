...
var newProducts = [
  ...
  newProduct.filter((val,index) => {
    if(index<2){
      if(val.old_price){
        $(`
        <ul  class="item" data-name=${val.name1} data-id=${val.id}>
          <li>
            <a href=""
              ><img
                src=${val.img}
...
// search products
  $("#searchProducts").keyup(function (e) { 
    const text = $(this).val();
    const products = newProducts.filter((val)=>
      val.name1.toLowerCase().includes(text.toLowerCase())
    );
    ...
    if(index<2){
      if(val.old_price){
        ...
        <ul  class="item" data-name=${val.name1} data-id=${val.id}>
          ...
          <li><a href="">${val.name1.split(text).join(`<span style="color: red">${text}</span>`)}</a></li>
          <li><a href="">${val.species}</a></li>
          <li><span class="old-price">$${val.old_price}.00</span><span class="price">$${val.price}.00</span></li>
        </ul>
        `).appendTo(".newProduct-group-item1");
...
$(document).on("click",".add_newProducts", function () {
  ...
  // const name =$(this).
  const name = $(this).parents(".item").data("name");
  console.log(name);
